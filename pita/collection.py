...
class Collection:
    ...
    def add_annotation(self, fname, fformat, prefix=""):
        ...
        if fformat in ["gtf", "gff3"]:
            c = read_gff(fname, fformat, collection=c, prefix=prefix)
             ...
             read_bed(fname, collection=c, prefix=prefix)
    ...
    def link_exons(self, e1, e2, ev=None):
        try:
            ...
            sys.stderr.write("refusing to link %s and %s, overlap or wrong order\n" % (e1, e2))
    ...
    def link_transcript_exons(self, transcript):
        ...
        for i in range(len(exons) - 1):
            try:
                ...
                sys.stderr.write("Linking {0} to {1}\n".format(exons[i], exons[i + 1]))
            except:
