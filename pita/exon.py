class Exon:
    ...
    def add_link(self, exon, ev):
        if exon.chr != self.chr:
            sys.stderr.write("%s:%s-%s %s - %s:%s-%s %s\n" % (self.chr, self.start, self.end, self.strand, exon.chr, exon.start, exon.end, exon.strand))
    ...
    def get_linked_exons(self, ev=None):
        if ev:
            return [k for k,v in self.links.items() if v == ev]
        else:
